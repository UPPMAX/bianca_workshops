...
- Docker requires root privileges and therefore run well on local computers or a cloud.

- HPC-clusters without root privileges uses Singularity/Apptainer.
...
??? note "(If not done already) Mount the wharf of your project."

    ```bash
    user@transit:~$ mount_wharf sens2025598
    Mounting wharf (accessible for you only) to /home/<user>/sens2025598
    <user>-sens2025598@bianca-sftp.uppmax.uu.se's password:
    ```
...
```bash
cd /proj/sens2025598/nobackup/wharf/$USER/$USER-sens2025598
```

- Move it to the project folder (good practice)

```bash
mv gatk_4.6.2.0.sif /proj/sens2025598/$USER/
```

- Go there

```bash
cd /proj/sens2025598/$USER/
```
...
???- tip "Set the Apptainer cache directories to ``/proj/<proj-id>`` to prevent quota issues"
    ...
    ```bash
    mkdir -p /proj/sens2025598/$USER/apptainer_cache_dir/{cache,tmp,localcache}
    export APPTAINER_CACHEDIR=/proj/sens2025598/$USER/apptainer_cache_dir/cache
    export APPTAINER_TMPDIR=/proj/sens2025598/$USER/apptainer_cache_dir/tmp
    export APPTAINER_LOCALCACHEDIR=/proj/sens2025598/$USER/apptainer_cache_dir/localcache
    ```
