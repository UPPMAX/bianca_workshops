...
read_data <- function() {
  readr::read_csv("evaluation.csv", show_col_types = FALSE)
}
...
comments <- t |>
  dplyr::select(starts_with("Any other feedback?"))
...
t <- t |> dplyr::select(dplyr::starts_with("Give you confidence levels of the following statements: "))
