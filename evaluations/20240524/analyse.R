...
read_data <- function() {
  readr::read_csv("20240524.csv", show_col_types = FALSE)
}
...
t <- t |> dplyr::select(dplyr::starts_with("Share your"))

# Rename confidence questions
names(t) <- stringr::str_sub(
  stringr::str_extract(names(t),  "\\[.*\\]"),
  start = 2,
  end = -2
)
